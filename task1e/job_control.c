...
job* initialize_job(char* cmd){
    struct job* _job = malloc(sizeof(job));
    _job -> cmd = malloc((sizeof(cmd)+1)* sizeof(char));
    strcpy(_job -> cmd, cmd);
    _job -> idx = 1;
    _job -> pgid = 0;
    _job -> status = 0;
    _job -> tmodes = malloc(sizeof(struct termios));
    _job -> next = NULL;
    return _job;
...
job* find_job_by_index(job* job_list, int idx){
    ...
    while(curr != NULL){
        if(curr -> idx == idx) return curr;
        curr = curr -> next;
    }
    printf("%s %d %s\n", "ERROR! can't find index",idx,"in list"); 
    return NULL;
...
void update_job_list(job **job_list, int remove_done_jobs){
    int ret_status;
    struct job* curr = job_list[0];
    while(curr != NULL){
        if(curr -> status == RUNNING){
            ret_status = waitpid(-curr -> pgid,&w_status,WNOHANG);
            if(ret_status == DONE) curr -> status = DONE;
        }
        curr = curr -> next;
    }
    if(remove_done_jobs){
        ...
        while(curr != NULL){
            if(curr -> status == DONE){
                printf("[%d]\t %s \t\t %s\n", curr->idx, status_to_str(curr->status),curr -> cmd); 
...
void run_job_in_foreground (job** job_list, job *j, int cont, struct termios* shell_tmodes, pid_t shell_pgid){
    pid_t ret_status = waitpid(-j -> pgid, NULL, WNOHANG); 
    if(ret_status == DONE){
        j -> status = DONE;
        printf("[%d]\t %s \t\t %s\n", j->idx, status_to_str(j->status),j -> cmd); 
    ...
    if(waitpid(-j -> pgid,&w_status,WUNTRACED)==DONE) j->status = DONE;
    else if(WIFSTOPPED(w_status))j -> status = SUSPENDED;
    else if(WIFSIGNALED(w_status)) {
        if(WTERMSIG(w_status) == SIGINT) j -> status = DONE;
    }
